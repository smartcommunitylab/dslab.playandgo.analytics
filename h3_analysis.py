...
def get_duck_avg_duration(campaign_id:str, time_slot:str, group_id:str, h3_cell:str, target_resolution:int, 
                              is_departure:bool, duck_engine: DuckEngine) -> pd.DataFrame:
    query = f"""
        SELECT player_id, multimodal_id, duration, h3_start, h3_end FROM trip_info
    ...
    df_h3 = pd.DataFrame(results, columns=['player_id', 'multimodal_id', 'duration', 'h3_start', 'h3_end'])
    ...
    if is_departure:
        ...
        df_agg = df_filtered.groupby('h3_end_parent', as_index=False).agg(
            tracks=('multimodal_id', 'nunique'),
            avg_duration=('duration', 'mean')
        )
    else:
        df_filtered = df_h3[df_h3['h3_end_parent'] == h3_cell]
        # Raggruppa per cella h3 e conta gli utenti unici
        df_agg = df_filtered.groupby('h3_start_parent', as_index=False).agg(
            tracks=('multimodal_id', 'nunique'),
            avg_duration=('duration', 'mean')
        )
    return df_agg
