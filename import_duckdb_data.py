...
if __name__ == "__main__": 
    ...
    #    size, df = storage_engine.load_dataframe(territory_id, storage_engine.nearest_edges, "2025")
    #    duck_df = duck_engine.convert_nearest_edges(territory_id, df, h3_level)
    #    storage_engine.save_df(territory_id, storage_engine.duck_nearest_edges, duck_df, "2025", True)
    ...
    for territory_id in territory_ids:
        size, df = storage_engine.load_dataframe(territory_id, storage_engine.mapped_campaign_groups, "2025")
        duck_df = duck_engine.convert_campaign_track_group_info(territory_id, df)
        storage_engine.save_df(territory_id, storage_engine.duck_tracks_info, duck_df, "2025", True)
        rows, columns = duck_df.shape
